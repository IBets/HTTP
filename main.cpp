...
namespace Wargaming {
	...
	auto Command::Execute(Context & cmdContext) -> JSON {
		...
		ostreamRequest << "application_id" << "=" << cmdContext.m_ID << "&";
...
int main(int argc, char* argv[]) {
	...
	try {
		...
		auto const accountID    = cmdContext.ExecuteCommand(cmdList.AppendParam("search", "Straik")).get_child("data").front().second.get<std::string>("account_id");
		auto const accountInfo  = cmdContext.ExecuteCommand(cmdInfo.AppendParam("account_id", accountID)).get_child("data").get_child(accountID);
		auto const accountTanks = cmdContext.ExecuteCommand(cmdTanks.AppendParam("account_id", accountID)).get_child("data").get_child(accountID);
